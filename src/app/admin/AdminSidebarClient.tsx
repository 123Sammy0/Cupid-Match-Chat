...
export default function AdminSidebarClient() {
  ...
  // Listen for toggle button click from the server-rendered header
  useEffect(() => {
    ...
    <>
      ...
      <aside
        className={`
          ...
          w-64 flex-shrink-0 bg-zinc-900 border-r border-zinc-800 flex flex-col
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 flex flex-col gap-1">
          <NavLink href="/admin" icon="dashboard" active={pathname === "/admin"}>Dashboard</NavLink>
          <NavLink href="/admin/users" icon="users" active={pathname === "/admin/users"}>Users & Roles</NavLink>
          <NavLink href="/admin/chats" icon="message-square" active={pathname === "/admin/chats"}>Chat Monitor</NavLink>
          <NavLink href="/admin/storage" icon="hard-drive" active={pathname === "/admin/storage"}>Storage Mgt.</NavLink>
          <NavLink href="/admin/rooms" icon="box" active={pathname === "/admin/rooms"}>Gate & Access</NavLink>
          
          <div className="my-4 border-t border-zinc-800 mx-2"></div>
          
          <NavLink href="/admin/health" icon="activity" active={pathname === "/admin/health"}>System Health</NavLink>
          <NavLink href="/admin/trash" icon="trash-2" active={pathname === "/admin/trash"}>Trash & Recovery</NavLink>
          <NavLink href="/admin/logs" icon="list" active={pathname === "/admin/logs"}>Audit Logs</NavLink>
          <NavLink href="/admin/settings" icon="settings" active={pathname === "/admin/settings"}>Global Settings</NavLink>
        </nav>
...
function NavLink({ href, icon, children, active = false }: { href: string; icon: string; children: ReactNode; active?: boolean }) {
  return (
    ...
    >
      <Icon name={icon} className={active ? "text-white" : "group-hover:text-white"} />
      <span>{children}</span>
    </Link>
  );
}
